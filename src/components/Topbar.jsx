    ...
    <div className="topbar">
      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
        ...
        <div style={{ display: 'flex', gap: 2, background: 'var(--bg-secondary)', border: '1px solid var(--border)', borderRadius: 8, padding: 3 }}>
          {['Linux', 'Windows', 'Mac'].map((os, i) => (
            <button key={os} style={{ padding: '3px 10px', borderRadius: 6, border: 'none', fontSize: 10, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s', background: i === 0 ? 'var(--accent-blue)' : 'transparent', color: i === 0 ? '#fff' : 'var(--text-muted)' }}>{os}</button>
          ))}
        </div>
      </div>
